...
def MaximizeMI_memsaver(
        seq_mat,seq_mat2,df,emat_0,split_point,wtrow,wtrow2,db=None,burnin=1000,iteration=30000,thin=10,
    ...
    def emat(p=pymcdf,value=emat_0):  
        ...
        v1 = numerics.eval_modelmatrix_on_mutarray(
            np.transpose(emat1),seq_mat,wtrow)
        v2 = numerics.eval_modelmatrix_on_mutarray(
...
def main(df,df2,lm='IM',modeltype='MAT',LS_means_std=None,\
    ...
    if lm == 'IM':
        ...
        emat = MaximizeMI_memsaver(
                seq_mat,seq_mat2,df.copy(),emat_0,split_point,wtrow,
                wtrow2,db=db,
                iteration=iteration,burnin=burnin,
                thin=thin,runnum=runnum,verbose=verbose)
...
def add_subparser(subparsers):
    p = subparsers.add_parser('learn_model_split_single_dataset')
    p.add_argument(
