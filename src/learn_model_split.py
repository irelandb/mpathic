...
def MaximizeMI_memsaver(
        seq_mat,seq_mat2,df,df2,emat_0,split_point,wtrow,wtrow2,db=None,burnin=1000,iteration=30000,thin=10,
        runnum=0,verbose=False):
    '''Performs MCMC MI maximzation using pymc.'''    
    n_seqs = seq_mat.shape[0]
    n_seqs2 = seq_mat2.shape[0]
    ...
    @pymc.stochastic(observed=True,dtype=pd.DataFrame)
    def pymcdf2(value=df2):
        return 0

    @pymc.stochastic(dtype=float)
    def emat(p=pymcdf,p2=pymcdf2,value=emat_0):  
        #need special function to evaluate pairwise models bc they are 1-D.
        emat1 = value[:,:split_point]
        emat2 = value[:,split_point:]        
        p['val'] = numerics.eval_modelmatrix_on_mutarray(
            np.transpose(emat1),seq_mat,wtrow)
        p2['val'] = numerics.eval_modelmatrix_on_mutarray(
            np.transpose(emat2),seq_mat2,wtrow2)            
        MI1 = EstimateMutualInfoforMImax.alt4(p.copy())
        MI2 =  EstimateMutualInfoforMImax.alt4(p2.copy())
        return n_seqs*MI + n_seqs2*MI2
...
def main(df,df2,lm='IM',modeltype='MAT',LS_means_std=None,\
    ...
    verbose=False,tm=None,start2=0,end2=0):
    ...
    df2.loc[:,seq_col_name] = df2.loc[:,seq_col_name].str.slice(start2,end2)
    df2 = utils.collapse_further(df2)
    ...
    if lm == 'IM':
        ...
        seq_mat2,wtrow2 = numerics.dataset2mutarray(df2.copy(),modeltype)
        #for info max we need to first initialize an initial guess for
        #the model matrix.
        #guess randomly
        seq_length = len(df[seq_col_name][0])
        seq_length2 = len(df2[seq_col_name][0])
        emat2_0 = None
        if initialize == 'rand':
            emat_0 = utils.gen_starting_emat(seq_length+seq_length2,len(seq_dict))
        ...
        emat = MaximizeMI_memsaver(
                seq_mat,seq_mat2,df.copy(),df2.copy(),emat_0,split_point,wtrow,
                wtrow2,db=db,
                iteration=iteration,burnin=burnin,
                thin=thin,runnum=runnum,verbose=verbose)
...
def wrapper(args):
    ...
    inloc2 = io.validate_file_for_reading(args.df2_name)
    input_df2 = io.load_dataset(inloc2)
    
    outloc = io.validate_file_for_writing(args.out) if args.out else sys.stdout
    #pdb.set_trace()

    output_df = main(input_df,input_df2,lm=args.learningmethod,\
...
def add_subparser(subparsers):
    p = subparsers.add_parser('learn_model_split')
    p.add_argument(
    ...
    p.add_argument(
        '-s2','--start2',type=int,default=0,
        help ='Position to start your analyzed region')
    p.add_argument(
        '-e2','--end2',type=int,default = None,
        help='Position to end your analyzed region')
    p.add_argument(
    ...
    p.add_argument(
        ...
        of stdin''')
    p.add_argument(
        '-df2','--df2_name',default=False,help='''read_second_input_file''')
