...
Overview
-------------
``profile_ct`` is a program within the sortseq package which calculates the
counts of each base (or amino acid) at each position in the library.
...
Example Command to Run the analysis::
    ...
    sortseq profile_ct -i data.txt -o counts.txt
